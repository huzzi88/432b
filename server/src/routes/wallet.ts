...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    // ── CASE 3: New email + new wallet → create new account
    let referredBy: string | null = null;
    if (referral_code) {
      const ref = await pool.query('SELECT id, referral_code FROM users WHERE referral_code = $1', [referral_code.toUpperCase()]);
